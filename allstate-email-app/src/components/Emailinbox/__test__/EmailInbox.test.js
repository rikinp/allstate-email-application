...
describe('PermanentDrawerLeft component', () => {
  const emailData = [
    { id: 1, subject: 'Test email 1', email: 'test1@test.com', message: 'This is a test email 1', seen: true },
    { id: 2, subject: 'Test email 2', email: 'test2@test.com', message: 'This is a test email 2', seen: false },
  ];

  it('renders the search input field', () => {
    render(<PermanentDrawerLeft emailData={emailData} />);
    const searchInput = screen.getByPlaceholderText('From:');
...
describe('PermanentDrawerLeft', () => {
    ...
    // test('calls onEmailSelected when email is clicked', async() => {
    //   const emailElement = await screen.findByText(emailData[0].subject);
    //   fireEvent.click(emailElement);
    //   expect(onEmailSelected).toHaveBeenCalledWith(emailData[0]);
    // });
    ...
    // test('calls deleteEmail when delete button is clicked', () => {
    // //   const deleteButton = screen.getByLabelText('delete');
    //   const deleteButton = screen.getByDisplayValue({target : { name: "DeleteButton" }});
